<script lang="ts">
  ...
  function changeIssueSectionTitle(event: EventElements, id?: string | number) {
    issueSections = issueSections.map(elem => {
      if (elem.issueSectionId === id) return { ...elem, title: event.currentTarget.value };
      return elem;
    });
  }
  ...
  function changeIssueSectionalOptionBody(event: EventElements, id?: string | number) {
    issueSectionalOptions = issueSectionalOptions.map(elem => {
      if (elem.issueSectionalOptionId === id) return { ...elem, body: event.currentTarget.value };
      return elem;
    });
  }
        ...
        {#key issueSectionalOptions}
          {#each getIssueSectionalOptionsWithSectionId(issueSectionId) as { issueSectionalOptionId, body } (issueSectionalOptionId)}
            ...
            <input
              ...
              placeholder="選択肢"
              value={body}
              on:change={e => changeIssueSectionalOptionBody(e, issueSectionalOptionId)}
            />
            <input
              type="button"
              value="選択肢を削除"
              on:click={e => removeIssueSectionalOption(e, issueSectionalOptionId)}
            />
          {:else}
            <p>選択肢を追加してください</p>
          {/each}
