...
function setValue( value: roomType, {
  roomId,
  guestUsers,
  hostUsers,
  inVoting,
  ...
  return {
    ...
    currentIssueSectionalOptionId: currentIssueSectionalOptionId ?? value.currentIssueSectionalOptionId,
  }
}
...
export function webSocketRouter(socket: Socket, io: Server) {
  socket.on('host:connect', async (roomId: string, sessionId: string) => {
    ...
    if (!value.hostUsers?.includes(sessionId)) {
      value = setValue(value, { hostUsers: [...(value.hostUsers || []), sessionId]});
      setRoomValue({ roomId, value });
    }

    emitAllUser({ socket, value, roomId });
  });

  socket.on('guest:connect', async (roomId: string, sessionId: string, guestName: string) => {
    let value = await getRoomValue({ roomId });

    if (!value.hostUsers?.includes(sessionId)) {
      const guestUsers = [...(value.guestUsers || []), { sessionId: guestName }];
  ...
  socket.on('host:update_settings', async () => {
